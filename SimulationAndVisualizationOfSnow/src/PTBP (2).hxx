...
#define PTBP_CAMERA_MAXVERTS 1001
#define PTBP_LIGHT_AVGVERTS 20

class PTBP : public AbstractRenderer
{
	...
	// Range query used for PPM, BPM, and PTBP. When HashGrid finds a vertex
	...
	public:
		...
		void Process(const UPBPLightVertex& aLightVertex)
		{
			...
			if ((aLightVertex.mPathLength + mCameraState.mPathLength > mPTBP.mMaxPathLength) ||
				(aLightVertex.mPathLength + mCameraState.mPathLength < mPTBP.mMinPathLength))
				return;
			...
			if (mPTBP.mAlgorithm != kPPM)
			{
				...
				const float wCamera = mPTBP.AccumulateCameraPathWeight2(mCameraState.mPathLength, misWeightFactorInv, sinTheta, aLightVertex.mMisData.mRaySamplePdfInv, aLightVertex.mMisData.mRaySamplePdfsRatio, cameraBsdfRevPdfW);
				const float wLight = mPTBP.AccumulateLightPathWeight2(aLightVertex.mPathIdx, aLightVertex.mPathLength, misWeightFactorInv, 0, 0, 0, cameraBsdfDirPdfW, aLightVertex.mInMedium ? PP3D : SURF, false);
...
public:
	...
	virtual void RunIteration(int aIteration)
	{
		...
		if (!(mEstimatorTechniques & SPECULAR_ONLY))
		{
			...
			UPBP_ASSERT(mMaxPathLength < PTBP_CAMERA_MAXVERTS);

			const float maxLightVerts = std::min(mLightSubPathCount * std::min((int)mMaxPathLength, PTBP_LIGHT_AVGVERTS), (float)mMaxMemoryPerThread / sizeof(UPBPLightVertex));
			const float maxBeams = std::min(mBB1DUsedLightSubPathCount * std::min((int)mMaxPathLength, PTBP_LIGHT_AVGVERTS), (float)mMaxMemoryPerThread / sizeof(UPBPLightVertex));
			...
			if (mMaxPathLength > 1)
			{
				...
				{
					Evaluator.build(mPhotonBeamsArray, mBB1DRadiusCalculation, radiusBB1D, mBB1DRadiusKNN, mVerbose);
				...
				for (;; ++cameraState.mPathLength)
				{
					...
					if (!mScene.Intersect(ray, originInMedium ? AbstractMedium::kOriginInMedium : 0, mRng, isect, cameraState.mBoundaryStack, mVolumeSegments, mLiteVolumeSegments))
					{
						...
						if (mMergeWithLightVerticesBB1D && !mPhotonBeamsArray.empty() && !stopBB1D)
						{
							...
							const Rgb contrib = Evaluator.evalBeamBeamEstimate(BeamType::LONG_BEAM, ray, mVolumeSegments, estimatorTechniques, originInMedium ? AbstractMedium::kOriginInMedium : 0, &data);
					...
					if (mMergeWithLightVerticesBB1D && !mPhotonBeamsArray.empty() && !stopBB1D)
					{
						...
						if (isect.IsOnSurface())
							contrib = Evaluator.evalBeamBeamEstimate(BeamType::LONG_BEAM, ray, mVolumeSegments, estimatorTechniques, originInMedium ? AbstractMedium::kOriginInMedium : 0, &data);
						else
							contrib = Evaluator.evalBeamBeamEstimate(BeamType::LONG_BEAM, ray, mLiteVolumeSegments, estimatorTechniques, originInMedium ? AbstractMedium::kOriginInMedium : 0, &data);
						const Rgb mult = cameraState.mThroughput * mBB1DNormalization;
						color += mult * contrib;
						mDebugImages.addAccumulatedLightSample(cameraState.mPathLength, DebugImages::BB1D, screenSample, mult);
					}
...
private:
	...
	PTBPEvaluator		 Evaluator;           // Encapsulates evaluating contributions of their intersections with beams
	...
	MisData mCameraVerticesMisData[PTBP_CAMERA_MAXVERTS]; // Stored MIS data for camera vertices (we don't need store whole vertices as for light paths)
