		...
		if (!(mEstimatorTechniques & SPECULAR_ONLY))
		{
			...
			if (mTraceLightPaths && mScene.GetLightCount() > 0 && mMaxPathLength > 1)
				for (int pathIdx = 0; pathIdx < pathCountL; pathIdx++)
				{
					...
					lightState.mThroughputEnd = lightState.mThroughput;
					...
					for (;; ++lightState.mPathLength)
					{
						...
						if (!mVolumeSegments.empty())
						{
							...
							// Attenuation
							lightState.mThroughput = lightState.mThroughputEnd;
						...
						if (mMergeWithLightVerticesBB1D && pathIdx < mBB1DUsedLightSubPathCount)
						{
							...
							AddBeams(ray, lightState.mThroughput, lightState.mThroughputEnd, &mLightVertices.back(), originInMedium ? AbstractMedium::kOriginInMedium : 0, lightState.mLastPdfWInv);
							if (!mPhotonBeamsArray.empty()) {
								mPhotonBeamsArray.back().startTime = previusDistance + time;
								mPhotonBeamsArray.back().ior = ior;
							}
						}
						...
						BSDF bsdf(ray, isect, mScene, BSDF::kFromLight, ior);
		...
		if (mTraceCameraPaths)
			for (int pathIdx = 0; pathIdx < pathCountC; ++pathIdx)
			{
				...
				for (;; ++cameraState.mPathLength)
				{
					...
					BSDF bsdf(ray, isect, mScene, BSDF::kFromCamera, ior);
...
private:
	...
	void GenerateLightSample(int aPathIdx, SubPathState& oLightState)
	{
		...
		lightVertex.mThroughput = oLightState.mThroughput;
	...
	)
	{
		...
		Rgb throughputEnd = aThroughputEnd;
		float raySamplePdf = 1.0f;
		float raySampleRevPdf = 1.0f;
		if (mPhotonBeamType == SHORT_BEAM)
		{
			for (VolumeSegments::const_iterator it = mVolumeSegments.cbegin(); it != mVolumeSegments.cend(); ++it)
			{
				...
				if (beam.mMedium->HasScattering() && (beam.mMedium->GetMeanFreePath(aRay.origin) > mBB1DMinMFP))
				{
					...
					beam.mThroughputAtEnd = throughputEnd;
